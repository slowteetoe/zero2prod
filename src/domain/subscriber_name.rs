...
impl SubscriberName {
    /// Returns an instance of 'SubscriberName' if the input satisfies all our validation constraints on subscriber name
    /// Panics otherwise
...
mod tests {
    ...
    fn a_valid_name_is_parsed_successfully() {
        let name = "José Cuervo".to_string();
    ...
    fn names_containing_invalid_char_are_rejected() {
        for name in &['/', '(', ')', '"', '<', '>', '\\', '{', '}'] {
            let name = format!("Some{}Name", name);
            assert_err!(SubscriberName::parse(name));
        }
    }
}
